...
impl<'a> Model {
    fn derive_legend_name(&self) -> String {
        ...
        format!("{}-{} {} - {}", first_date.year(), last_date.format("%y"), reservoir.dam, station_id)
    }
    pub fn generate_svg(
        &self,
        ...
        if let Some(mut normalized_water_years) = self.reservoir_data.get_clean_reservoir_water_years(self.selected_reservoir) {
            let ranged_date = NormalizedNaiveDate::get_normalized_ranged_date();
            match self.selected_sort {
                Msg::SelectedSort(SortBy::DriestYears) => normalized_water_years.sort_by_lowest_recorded_years(),
            ...
            let y_max = normalized_water_years.get_largest_acrefeet_over_n_years(NUMBER_OF_CHARTS_TO_DISPLAY_DEFAULT);
            let colors_for_water_years = get_colors(NUMBER_OF_CHARTS_TO_DISPLAY_DEFAULT).unwrap();
            let plot_and_color = normalize_calendar_years.iter().zip(colors_for_water_years.iter());
            ...
            for (water_year, rgb_color) in plot_and_color {
                    let legend_title = self.derive_legend_name(water_year);
                    chart
                    .draw_series(LineSeries::new(
                            ...
                            .iter()
                            .map(|survey| {
                                let normalized_date_observation: NormalizedNaiveDate = survey.tap().date_observation;
