...
/// A single measurement sample from a CDEC station.
...
/// A typed observation that distinguishes between daily and monthly measurements.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum Survey {
    Daily(Tap),
    Monthly(Tap),
...
/// Date format for compressed string records: "YYYYMMDD"
pub const YEAR_FORMAT: &str = "%Y%m%d";

pub type DailySurvey = Survey;
pub type MonthlySurvey = Survey;

/// A compressed representation of a survey as a CSV StringRecord.
/// Format: station_id, duration(D/M), date(YYYYMMDD), value
pub struct CompressedStringRecord(pub StringRecord);

/// A cumulative sum record as a CSV StringRecord.
/// Format: date(YYYYMMDD), cumulative_value
pub struct CumulativeSummedStringRecord(pub StringRecord);

/// Convert a vector of CompressedStringRecords into Surveys.
pub trait VectorCompressedStringRecord {
    fn records_to_surveys(self) -> Vec<Survey>;
}

/// Convert a vector of CumulativeSummedStringRecords into (date, value) tuples.
...
/// Trait for interpolating between pairs of surveys to fill in missing daily data.
...
impl Tap {
    /// Convert the tap's value to f64. Non-recording values return 0.0.
    pub fn value_as_f64(&self) -> f64 {
        match self.value {
            DataRecording::Recording(a) => a as f64,
            _ => 0.0f64,
        }
    }
}
...
impl Interpolate for (Survey, Survey) {
    fn interpolate_pair(self) -> Option<Vec<Survey>> {
        ...
        // just force a daily observation
        ...
        let x_n: f64 = days as f64;
        let x_0: f64 = 0.0;
        let slope = (y_n - y_0) / (x_n - x_0);
        for idx in 1..=days {
            let fdx: f64 = idx as f64;
...
impl Survey {
    /// Get a mutable reference to the inner Tap.
    ...
    /// Get the month datum (year, month) for this survey.
    ...
    /// Get a reference to the inner Tap.
    ...
    /// Get the recording value as f64. Non-recording values return 0.0.
    ...
    /// Returns true if this survey contains an actual recording value.
