...
/// Trait to group survey and observable types, enabling conversion to Survey.
pub trait Observable: Clone {
    fn into_survey(self) -> Survey;
}

/// Represents a (year, month) pair for grouping observations by month.
#[derive(Debug, Clone)]
pub struct MonthDatum(u32, u32);

impl MonthDatum {
    /// Create a MonthDatum from a NaiveDate.
    ...
    /// Create a MonthDatum from explicit year and month values.
...
/// Trait for building a compressed survey range by merging daily and monthly data.
...
/// A range of observations with start/end dates and a set of months that have data.
...
/// Trait for running CSV generation on a collection of ObservableRanges.
    ...
    fn run_csv_v2(&self) -> String {
        ...
        for observable_range in observations_downloaded {
            for survey in observable_range.observations {
                ...
                let reservoir_capacity: f64 = reservoir.capacity as f64;
                let observed_value = {
...
/// Trait for interpolating all observations within a collection of ObservableRanges.
